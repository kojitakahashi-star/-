...
### 既知の制約（2026-08-02 時点で検証済み）

1. **MCP コネクタは高橋さん本人の名義で投稿する。** そのため本文の `<@U063SJG0N0L>` は
   自分から自分へのメンションになり、**Slack の通知が鳴らない**。
2. **実行環境から `slack.com` / `hooks.slack.com` へ出られない**（組織のegressポリシーで
   CONNECT が 403）。そのため bot token / Webhook はポリシーが変わるまで使えない。

→ この2つが解消されるまで、Slack のメンション通知を鳴らす手段は無い。
   代わりに **Routine の完了通知（プッシュ／メール）** で気付ける状態にしてある。
   Slack への投稿は「記録」として続ける。
   egress が許可されたら `SLACK_WEBHOOK_URL` を設定するだけで経路2が有効になる。

### 大前提: 誰の名義で投稿するかで通知が鳴るかが決まる

| 経路 | 投稿名義 | メンション通知 | スレッド返信 |
| --- | --- | --- | --- |
| **1. bot token** (`post_slack.py`) | Slackアプリ | **鳴る** | できる |
| **2. Webhook** (`post_slack.py`) | Slackアプリ | **鳴る** | できない（連投） |
| 3. MCP `slack_send_message` | **高橋さん本人** | **鳴らない** | できる |

MCP コネクタは高橋さん本人のアカウントとして投稿するため、本文の `<@U063SJG0N0L>` が
**自分から自分へのメンション**になり、Slack の仕様で通知が鳴らない（検証済み）。
**経路3は通知手段として使えない。** 必ず経路1/2 を優先すること。
...
`SLACK_BOT_TOKEN` があれば bot token で（スレッド返信つき）、無くて
`SLACK_WEBHOOK_URL` があれば Webhook で投稿する。どちらも無ければ非ゼロ終了する。
**成功したらここで終わり。**手順6の `--delivered` へ進む。

### 経路3（最後の手段）: MCP コネクタ

経路1・2 がどちらも使えなかったときだけ使う。**通知は鳴らないので「記録」でしかない。**
それでもチャンネルに情報を残す価値はあるので投稿する。
...
- **本文の先頭に次の1行を必ず足す**（通知が鳴っていないことを気付けるようにするため）:

  `:warning: _この投稿は本人名義のため、メンションしても通知が鳴りません。SLACK_BOT_TOKEN か SLACK_WEBHOOK_URL を設定してください。_`

- 経路3で投稿した場合も **「配信成功」扱い**にする（同じ内容を次回に再送しても
  通知は鳴らず、チャンネルが重複で埋まるだけなので）。ただし最終出力には
  「通知が鳴らない経路で投稿した」ことを明記する。

### 判定

- **経路1・2・3 のいずれかが成功 → 「配信成功」**（手順6の `--delivered` へ）
- **すべて失敗 → 「配信失敗」**（手順6の `--failed` へ。次回リトライされる）

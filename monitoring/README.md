...
**経路1（本命）: MCP コネクタ `slack_send_message`** — 実際に配信できることを確認済み。
高橋さん本人のアカウント名義で投稿し、スレッド返信も使える。

| 経路 | 投稿名義 | メンション通知 | スレッド | 現在 |
| --- | --- | --- | --- | --- |
| **1. MCPコネクタ** | 本人 | 鳴らない（未読にはなる） | できる | **本命・動作確認済み** |
| 2. bot token | Slackアプリ | 鳴る | できる | egressでブロック |
| 2. Webhook | Slackアプリ | 鳴る | できない | egressでブロック |

経路1ではメンションのプッシュ通知は鳴らない（Slackは自分の投稿では通知しない）。
チャンネルが未読になる形で運用する。

### あとからプッシュ通知を鳴らしたくなったら

組織のegressポリシーで **`hooks.slack.com` を許可**し、Slack で Incoming Webhook を発行して
環境変数 `SLACK_WEBHOOK_URL` に設定する。それだけで経路2に切り替わり、アプリ名義の投稿＝
メンション通知が鳴るようになる（コードは実装済み）。`slack.com` も許可できるなら
`SLACK_BOT_TOKEN` の方がスレッド返信も使えて望ましい。
